...
const { handleApprovalCreate, handleApprovalUpdate } = require('../services/approvalService');
  ...
  const eventDispatcher = new lark.EventDispatcher({}).register({
    ...
    'bitable.record.changed': async (data) => {
      try {
        ...
        if (tableId !== config.bitable.approvalTableId) {
          return;
        }

        if (changeType === 'add' || changeType === 'create') {
          await handleApprovalCreate(recordId);
        } else if (changeType === 'update' || changeType === 'modify') {
          await handleApprovalUpdate(recordId);
        }
      } catch (err) {
...
async function processBitableEvent(event) {
  ...
  if (table_id === config.bitable.approvalTableId) {
    if (action_type === 'create') {
      await handleApprovalCreate(record_id, fields);
    } else if (action_type === 'update') {
      await handleApprovalUpdate(record_id, fields);
    }
  }
}
