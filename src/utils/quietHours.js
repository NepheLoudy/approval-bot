...
//   - task（gateTask）：可重扫的任务（催办周报/每日待审批提醒/催发票私聊）
//     只登记名字+触发槽位，冲刷时重新执行整个任务函数——以补发时刻的最新
//     数据重查，夜里已了结的审批不再催，催办状态/已催标记/p2p 回复轮询等
//     时点逻辑都以实际补发时刻为准（「挤压要为挤压之后的事情负责」）。
//
// 积压持久化到项目根 .quiet-backlog.json：重启不丢。启动时已过 end 整点则
// 立即补冲刷，否则调度到 end 整点。冲刷失败的单条保留重试（至多 3 次尝试），
// 超限丢弃并打错误日志。
//
// 不受限：对话/指令回复与人工当下主动触发（/api/broadcast 等手动接口，
...
const payloadHandlers = {}; // 本项目无一次性载荷积压场景（定时任务均为可重扫任务）
