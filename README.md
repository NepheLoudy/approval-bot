...
|----------|---------|----------------|---------|
| 🧾 未交发票 | 发票栏为空（私聊另有门槛：完成时间满 `INVOICE_URGE_GRACE_DAYS` 天，默认 14；周报群卡片无时间门槛，列全量） | 发起人 / **私聊** IM API，附「申请编号」自带的**审批实例链接**（审批详情页，非表格链接）；财务 / 周报群卡片 | 每天 10:30 私聊；周一 18:00 周报；`/approval-urge 发票` |
