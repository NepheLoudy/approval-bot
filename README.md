...
└─ 每天 10:30（INVOICE_URGE_SCHEDULE，留空不启用）
     ...
     ↳ 私聊后独立群播「🔔 今日已催」卡（与周报能力分开）：
       ...
       回复得知无法提交、发起人已退队）/ 未私聊汇总；无催交且无重点关注则不发卡
```
...
**「今日已催」每日播报**（与周报能力分开，两张卡互不混装）：每天 10:30 私聊催交后（及 `/approval-urge` 手动触发发票分支时）向审批群独立推送，内容：① 今日已私聊催交明细（单号超链接+徽标）；② ⚠️ 需财务关注——多次催交仍无票（已催 ≥2 次，含已催满 3 次）、回复得知「无法提交」与通讯录校验发现发起人已退队的记录，附原因标签；③ 未私聊汇总（延期中/无法提交/已催满/已退队）。有需关注记录时卡片头为红色。无催交且无重点关注时不发卡。周报（周一 18:00）保持原有三段全量清单 + 本周统计，不包含今日已催内容。
...
│   │   ├── invoiceUrgeService.js  # 催发票私聊（回复轮询/通讯录兜底/状态过滤/计数升级，支持 dryRun）
