...
当前最新：**v53**（2026-09-26，随本提交落地；部署待实验室网段恢复后与 v49-v52 同批补上并回填哈希）。上一版 v52（财务协作指南梳理）。上一版 v50（台账同步）。上一版 v49（报销交付包，`f8ac36a`）。上一版 v48（batchOverview 排序修复+文档批，`24617de`）。上一版 v47（财务协作指南，`7ab3dad`）。上一版 v46（全量复查修复批）。
...
## v53 · 2026-09-26 · 随本提交落地 · fix

**七仓全量审查修复批（子代理审查+主会话甄别修复，资金路径优先）**

- 提交说明：fix: 全量审查修复——regen 排序/笔序并发锁/markBatch 状态机/操作留痕/接取互斥等
- **P1×3**：①regenerateBatchFiles 映射漏 collectedAt 致排序恒 0、regen 后打印件顺序漂移（违背「锁定后顺序不可变」）——票池/regen 映射收敛为共享 collectToItem 并补字段；②「第N笔」笔序在 per-batchNo 锁内计算，同项目并发锁定会重复笔序→台账按摘要匹配丢账——笔序+createBatch 段包 withLock('proj_<项目>') 嵌套锁；③markBatch 无状态机校验（已到账可再 reject=重复报销风险）——新增 BATCH_TRANSITIONS 合法流转表，非法流转抛错。
